...
class MTDecoderV1(MTBaseDecoder, quant_utils.QuantizableLayer):
  ...
  def Params(cls):
    ...
    p.Define('have_projection', False, 'Whether to have projection layer')
    p.Define('proj_dim', 512, 'Whether to have projection layer')
    p.Define('projection_train', False, 'Whether to train projection layer')

    ##### Experimental for ESN #####
    p.Define('input_scale', [5.0] * 6, 'ESN radius for each layer.')
    p.Define('spectral_radius', [0.9] * 6, 'ESN input scale for each layer.')
    p.Define('input_sparsity', [0.8] * 6, 'input sparsity for each layer.')
    p.Define('hidden_sparsity', [0.8] * 6, 'reservoir sparsity for each layer.')
    p.Define('softmax_in_dim', 0, 'Input dim to softmax.')

    p.Define('recurrent_dropout', True, 'Will the recurrent part have d')
    p.Define('reservoir_dim', 1024, 'size of reservoir.')
    p.Define('is_average_esn', False, 'Will the recurrent part have d')
    p.Define('is_concat_esn', False, 'Will the recurrent part have d')
    ...
    with tf.variable_scope(p.name):
      ...
      with tf.device(emb_device):
        ...
        for i in range(1, p.rnn_layers):
          ...
          params.name = 'rnn%d' % i
          params.input_scale = p.input_scale[i]
          params.spectral_radius = p.spectral_radius[i]
          params.input_sparsity = p.input_sparsity[i]
          params.hidden_sparsity = p.hidden_sparsity[i]
          params.dtype = p.dtype
          if i == 1:
            params.num_input_nodes = p.rnn_cell_dim + p.attention.source_dim
          else:
            params.num_input_nodes = p.rnn_cell_dim + p.attention.source_dim
            # params.num_input_nodes = 1536 + p.attention.source_dim
          params.num_output_nodes = p.rnn_cell_dim
          # params.num_output_nodes = 1536
          params.reset_cell_state = p.packed_input
          rnn_cell_p = params
      ...
      if p.feed_attention_context_vec_to_softmax:
        # p.softmax.input_dim = p.reservoir_dim + p.attention.source_dim
        p.softmax.input_dim = p.rnn_cell_dim + p.attention.source_dim
      else:
        p.softmax.input_dim = p.rnn_cell_dim
        # p.softmax.input_dim = 1536

      ######## For ESN experiment ########
      # if p.have_projection or p.softmax.input_dim != p.softmax_in_dim:
      if p.have_projection:
        # Project the decoder output to the desired dim.
        proj_p = p.proj_tpl.Copy().Set(
            name='proj',
            batch_norm=False,
            input_dim=p.softmax.input_dim,
            output_dim=p.proj_dim,
            trainable=p.projection_train,
            params_init=py_utils.WeightInit.Orthogonal())
        proj_p.has_bias = True
        proj_p.activation = 'NONE'
        self.CreateChild('final_proj', proj_p)
        p.softmax.input_dim = p.proj_dim
  ...
  def ComputePredictions(self, theta, encoder_outputs, targets):
    ...
    with tf.name_scope(p.name):
      ...
      with tf.device(emb_device):
        ...
        if p.is_average_esn:
          xs = tf.math.add_n(outputs_list) / (p.rnn_layers - 1)

        if p.is_concat_esn:
          xs = tf.concat(outputs_list, -1)
        ...
        if hasattr(self, 'final_proj'):
          # paddings = tf.reshape(target_paddings, [-1])
          xs = self.final_proj.FProp(theta.final_proj, xs, target_paddings)
  ...
  def _DecodeStep(self, theta, encoder_outputs, embs, step_paddings,
    ...
    for i, (layer, layer_theta) in enumerate(zip(self.frnn, theta.frnn)):
      ...
      if 1 + i >= p.residual_start:
        ...
        rnn_out = new_rnn_out
      outputs_list.append(new_rnn_out)

    if p.is_average_esn:
      rnn_out = tf.math.add_n(outputs_list) / (p.rnn_layers - 1)

    if p.is_concat_esn:
      rnn_out = tf.concat(outputs_list, -1)
  ...
  def _PreBeamSearchStepCallback(self, theta, encoder_outputs, step_ids, states,
    ...
    ######### For ESN experiments ########
    if hasattr(self, 'final_proj'):
      paddings = tf.reshape(step_paddings, [-1])
      step_out = self.final_proj.FProp(theta.final_proj, step_out,
