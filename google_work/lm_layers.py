...
class RnnLm(RnnLmNoEmbedding):
  ...
  def CommonParams(cls,
                   ...
                   train_wts=False,
                   order=1,
                   esn_init_dist='gaussian',
                   esn_output_mode='non_linear',
                   esn_layer_norm=False,
                   esn_input_scale=[5.0] * 6,
                   esn_radius=[0.9] * 6,
                   esn_train_scale=False,
                   esn_train_radius=True,
                   esn_input_sparsity=[0.8] * 6,
                   esn_hidden_sparsity=[0.8] * 6):
    ...
    if is_esn == 'random':
      ...
      for i in range(num_layers):
        esn_cell = esn.Copy().Set(
            input_scale=esn_input_scale[i],
            spectral_radius=esn_radius[i],
            input_sparsity=esn_input_sparsity[i],
            hidden_sparsity=esn_hidden_sparsity[i])
        p.rnns.cell_tpl.append(esn_cell.Copy())
    elif is_esn == 'topology':  # the esn topology cell
      # wt_topologies = 'lap_sw'  # 'lap_grid' # 'lap_chain' # 'rot' 'random'
      # train_wts = False
      ...
      for i in range(num_layers):
        esn_cell = esn.Copy().Set(
            ...
            hidden_sparsity=esn_hidden_sparsity[i])
        p.rnns.cell_tpl.append(esn_cell.Copy())
    elif is_esn == 'basis':  # ['basis', 'basis3D'] the esn with all basis cell
      # Note: change to basis and the rnn_cell to basis for simple topo expts
      # train_wts = False  # True
      esn = rnn_cell.ESNcellBasis.Params().Set(
          output_mode=esn_output_mode,  # 'non_linear',#'actNN',
          leak_weight=0.9,
          init_distribution=esn_init_dist,
          layer_norm=esn_layer_norm,
