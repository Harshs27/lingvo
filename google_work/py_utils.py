...
from google3.third_party.google_research.google_research.model_pruning.python import pruning
# pylint: disable=g-direct-tensorflow-import
from google3.third_party.tensorflow.core.framework import node_def_pb2
from google3.third_party.tensorflow.core.protobuf import rewriter_config_pb2
from google3.third_party.tensorflow.python.framework import func_graph
...
class WeightInit(object):
  ...
  def _Params(method,
              ...
              hidden_sparsity=0.8,
              input_nodes=100,
              weight_topology_in='random',
              weight_topology_res='random',
              get_mask=False,
              wt_seed=None):
    """Params function to initialize the weights."""
    p = hyperparams.Params()
    p.Define('method', method, 'Initialization method.')
    p.Define('scale', scale, 'Initialization scale.')
    p.Define('seed', seed, 'Random seed used to generate initial values.')
    ####### For ESN experiment. ########
    p.Define('input_scale', input_scale, 'Input scale.')
    p.Define('spectral_radius', spectral_radius, 'Spectral radius.')
    p.Define('input_sparsity', input_sparsity, 'Input sparsity.')
    p.Define('hidden_sparsity', hidden_sparsity, 'h sparsity.')
    p.Define('weight_topology_in', weight_topology_in, 'weight topology in.')
    p.Define('weight_topology_res', weight_topology_res, 'weight topology res.')
    p.Define('get_mask', get_mask, 'returns the binary mask of topology')
    p.Define('wt_seed', wt_seed, 'the seed used for weight init')
    ####### For LSTM experiment. ########
    p.Define('input_nodes', input_nodes, 'input nodes.')
    p.Freeze()
    return p
  ...
  @staticmethod
  def Projection(scale=1.0, seed=None):
    return WeightInit._Params('projection_initializer', scale, seed)

  @staticmethod
  def Orthogonal(scale=1.0, seed=None):
    return WeightInit._Params('orthogonal', scale, seed)

  @staticmethod
  def ESNFeedbackGaussian(scale=1.0, seed=None):
    """scale controls sparsity."""
    return WeightInit._Params('esn_feedback_gaussian', scale, seed)

  @staticmethod
  def ESNInGaussian(scale=1.0,
                    input_scale=1.0,
                    weight_topology_in='random',
                    get_mask=False,
                    seed=None,
                    wt_seed=None):
    """scale * ESN initialization for inputs."""
    return WeightInit._Params(
        'esn_in_gaussian',
        scale,
        seed=seed,
        input_scale=input_scale,
        weight_topology_in=weight_topology_in,
        get_mask=get_mask,
        wt_seed=wt_seed)

  @staticmethod
  def ESNResGaussian(scale=1.0,
                     spectral_radius=0.9,
                     weight_topology_res='random',
                     get_mask=False,
                     seed=None,
                     wt_seed=None):
    """scale * ESN initialization for reservoir."""
    return WeightInit._Params(
        'esn_res_gaussian',
        scale,
        seed=seed,
        spectral_radius=spectral_radius,
        weight_topology_res=weight_topology_res,
        get_mask=get_mask,
        wt_seed=wt_seed)

  @staticmethod
  def ESNInUniform(scale=1.0,
                   input_scale=1.0,
                   weight_topology_in='random',
                   get_mask=False,
                   seed=None,
  ...
  def LSTMUniform(scale=1.0,
                  ...
                  hidden_sparsity=0.8,
                  input_nodes=100):
    return WeightInit._Params('lstm_uniform', scale, seed, input_scale,
                              spectral_radius, input_sparsity, hidden_sparsity,
                              input_nodes)

  @staticmethod
  def LSTMGaussian(scale=1.0,
                   seed=None,
                   input_scale=1.0,
                   spectral_radius=0.9,
                   input_sparsity=0.8,
                   hidden_sparsity=0.8,
                   input_nodes=100):
    return WeightInit._Params('lstm_gaussian', scale, seed, input_scale,
                              spectral_radius, input_sparsity, hidden_sparsity,
                              input_nodes)
...
def CreateVariable(name,
  ...
  seed = p.init.seed
  ##### ESN experiment #####
  input_scale = p.init.input_scale
  spectral_radius = p.init.spectral_radius
  input_sparsity = p.init.input_sparsity
  hidden_sparsity = p.init.hidden_sparsity
  input_nodes = p.init.input_nodes
  weight_topology_res = p.init.weight_topology_res
  weight_topology_in = p.init.weight_topology_in
  get_mask = p.init.get_mask
  wt_seed = p.init.wt_seed
  avg_seq_len = 100  # average length of input sequences

  # Defining a function for the WeightInit class
  def GetComplementaryTopology(lap, dtype):
    """Function to get complementary topology."""
    # returns the laplacian complementary: to handle eigvecs <-> lesser eigvals
    eigval, eigvec = tf.linalg.eigh(lap)
    # reverse the eigval
    eigval_c = tf.reverse(eigval, axis=[0])
    # get the complementary laplacian
    lap_c = tf.matmul(
        tf.matmul(eigvec, tf.linalg.diag(eigval_c)), tf.transpose(eigvec))
    mask_c = tf.ones(lap_c.shape)
    mask_c = tf.cast(mask_c, dtype=dtype)
    lap_c = tf.cast(lap_c, dtype=dtype)
    return lap_c, mask_c

  def GetComplementaryTopologySVD(lap, dtype):
    """Function to get complementary topology using SVD decomposition."""
    # take the svd
  ...
  def GetClosestInt(r):
    """Closest integer factorization."""
    # r = f1 x f2, where f1 f2 are the closest ints
  ...
  def GetLaplacian(r, topo, seed=None):
    """Function to get laplacian matrix.

    Args:
      r: number of hidden nodes.
      topo: the type of laplacian topology.
      seed: the weight seed.

    Returns:
      Laplacian topology and mask is returned.
    """
    if '_chain' in topo:
      g = nx.grid_graph([r], periodic=True)
    elif '_grid' in topo:
      grid_dim = GetClosestInt(r)
      # print('grid dim ', grid_dim)
      g = nx.grid_graph(grid_dim, periodic=True)
    elif '_sw' in topo:  # small world
      mean_degree = int(np.ceil(0.1 * r)) + 1  # 10% of Total nodes
      # print('small world r, k ', r, mean_degree)
      rewiring_prob = 0.5  # randomly choose from the neigboring nodes
      g = nx.connected_watts_strogatz_graph(
          r, mean_degree, rewiring_prob, seed=seed)
    else:
      print('Laplacian topology not defined: ', topo)
    adj = nx.to_numpy_matrix(g)
    adj = tf.convert_to_tensor(adj)
    diag = tf.reduce_sum(adj, axis=-1)
    lap = tf.linalg.diag(diag) - adj
    # if r %2==0: # even
    mask = tf.linalg.diag(tf.ones(adj.shape[-1], dtype=adj.dtype)) + adj
    if r % 2 == 1 and '_grid' in topo:  # odd
      mask = adj
    return lap, mask

  def GetWeightTopologyMask(v, shape, dtype, weight_topology, wt_type, scale,
                            wt_seed):
    """Get the weight topology with its mask."""
    if weight_topology in ['random', 'random_basis']:
      if wt_type == 'IN':
        rand_seed = wt_seed + 7
      elif wt_type == 'RES':
        rand_seed = wt_seed + 11
      mask = tf.cast(
          tf.less_equal(tf.random.uniform(shape, seed=rand_seed), scale),
          dtype=dtype)
    elif weight_topology == 'scrl':  # scr with lower
      ...
      mask = tf.cast(w_scr, dtype=dtype)
    elif weight_topology == 'scru':  # scr with lower
      dim0, dim1 = shape[-2], shape[-1]
      if dim0 <= dim1:
        shape0, shape1 = dim0, dim1
        do_transpose = False
      ...
      if do_transpose:
        w_scr = tf.transpose(w_scr, perm=[1, 0])
      mask = tf.cast(w_scr, dtype=dtype)
    elif weight_topology == 'eye':  # scr with lower
      dim0, dim1 = shape[-2], shape[-1]
      if dim0 <= dim1:
        shape0, shape1 = dim0, dim1
        do_transpose = False
      ...
      if do_transpose:
        w_scr = tf.transpose(w_scr, perm=[1, 0])
      mask = tf.cast(w_scr, dtype=dtype)
    elif weight_topology == 'scrbd':  # scr with lower
      dim0, dim1 = shape[-2], shape[-1]
      if dim0 <= dim1:
        shape0, shape1 = dim0, dim1
        do_transpose = False
      ...
      for i in range(repeat):
        if i == repeat - 1:  # truncate
          ...
          w_scr = tf.concat([w_scr, tf.eye(shape0)], axis=1)
      w_scr = tf.roll(
          w_scr, shift=[-1, 0], axis=[0, 1]) + tf.roll(
              w_scr, shift=[1, 0], axis=[0, 1]) + w_scr
      # the transpose condition
      if do_transpose:
        w_scr = tf.transpose(w_scr, perm=[1, 0])
      mask = tf.cast(w_scr, dtype=dtype)
    elif weight_topology == 'rot':
      dim0, dim1 = shape[-2], shape[-1]
      ...
      if do_transpose:
        w_scr = tf.transpose(w_scr, perm=[1, 0])
      # rotation code
      num_rotates = math.ceil(shape0 / avg_seq_len)
      for r in range(num_rotates):
        temp = tf.roll(w_scr, shift=[r * avg_seq_len, 0], axis=[0, 1])
        if r == 0:
          w_rot = temp
        else:
          w_rot = w_rot + temp
      mask = tf.cast(w_rot, dtype=dtype)
    elif weight_topology == 'basis3D':  # scrl in a 3way tensor
      mask3d = []
      for rot in range(shape[-1]):  # shape = [in, res, num_basis]
    ...
    elif 'lap_' in weight_topology:  # for all networkx : grid, sw
      # rectangular mat
      ...
      # assuming shape0 < shape1:
      repeat = math.ceil(shape1 / shape0) - 1
      lap, mask = GetLaplacian(r=shape0, topo=weight_topology, seed=wt_seed)
      for i in range(repeat):
        temp_lap, temp_m = GetLaplacian(
            r=shape0, topo=weight_topology, seed=wt_seed)
        if i == repeat - 1:  # truncate
          lap = tf.concat([lap, temp_lap[:, :(shape1 - shape0)]], axis=1)
          mask = tf.concat([mask, temp_m[:, :(shape1 - shape0)]], axis=1)
        else:
          lap = tf.concat([lap, temp_lap], axis=1)
          mask = tf.concat([mask, temp_m], axis=1)
      # the transpose condition
      if do_transpose:
        lap = tf.transpose(lap, perm=[1, 0])
        mask = tf.transpose(mask, perm=[1, 0])
      # should have v and mask
      v = tf.cast(lap, dtype=dtype)
      mask = tf.cast(mask, dtype=dtype)
      if '_comp' in weight_topology:
        if wt_type == 'IN':  # input matrix
          v, mask = GetComplementaryTopologySVD(v, dtype)
        elif wt_type == 'RES':  # reservoir matrix
          v, mask = GetComplementaryTopology(v, dtype)
        else:
          print('CHECK the weight type : [IN, RES] ', wt_type)
    else:
  ...
  elif method in 'orthogonal':
    millis = int(round(time.time() * 1000))
    v_init = tf.orthogonal_initializer(seed=millis, dtype=dtype)

  elif method in ['projection_initializer']:

    def ProjectionInitializerInit(shape, dtype):  # , partition_info=None):
      dim0 = shape[0]
      dim1 = shape[1]
      std_dev = 1.0 / (math.sqrt(dim0))
      millis = int(round(time.time() * 1000))
      v = np.random.normal(0.0, std_dev, (dim0, dim1))
      tf.random_normal(
          shape, mean=0.0, stddev=std_dev, dtype=dtype, seed=millis)
      return v

    v_init = ProjectionInitializerInit

  elif method in ['esn_feedback_gaussian']:

    def ESNFeedbackGaussian(shape, dtype):  # , partition_info=None):
      """ESN Feedback Gaussian, returns the matrix with gaussian init."""
      dim0 = shape[0]
      # dim1 = shape[1]
      std_dev = 1.0 / (math.sqrt(dim0))
      millis = int(round(time.time() * 1000))
      v = tf.random_normal(
          shape, mean=0.0, stddev=std_dev, seed=millis + 5, dtype=dtype)
      mask = tf.cast(
          tf.less_equal(tf.random_uniform(shape, seed=millis + 7), scale),
          dtype=dtype)
      v = tf.multiply(v, mask)
      return v

    v_init = ESNFeedbackGaussian

  elif method in ['esn_in_uniform', 'esn_in_gaussian']:
    # pylint: disable=unused-argument
    def ESNIn(shape, dtype, partition_info):
      """ESN In function."""
      # nonlocal wt_seed
      # For ESN input. Note scale in this case constrols the density of
      # the reservoir matrix.
      if method == 'esn_in_uniform':
        v = tf.random.uniform(
            shape, minval=-1.0, maxval=1.0, seed=wt_seed + 5, dtype=dtype)
      elif method == 'esn_in_gaussian':
        v = tf.random.normal(
            shape, mean=0.0, stddev=1.0, seed=wt_seed + 5, dtype=dtype)
      # get the topology matrix with its mask
      v, mask = GetWeightTopologyMask(v, shape, dtype, weight_topology_in, 'IN',
                                      scale, wt_seed)
      v = tf.multiply(v, mask)

      if weight_topology_in in ['basis3D']:
        v_3d = []
        for num_b in range(shape[-1]):
          v_norm2 = tf.sqrt(tf.reduce_sum(tf.square(v[:, :, num_b])))
          is_norm_0 = tf.cast(tf.equal(v_norm2, 0), dtype=dtype)
          v_3d.append(
              tf.expand_dims(v[:, :, num_b] / (v_norm2 + 1 * is_norm_0), -1))
        v = tf.concat(v_3d, axis=-1)
      elif weight_topology_in in ['random_basis']:  # normalize each column
        v_normalized = []
        for num_col in range(shape[-1]):
          v_col = v[:, num_col]
          v_norm2 = tf.sqrt(tf.reduce_sum(tf.square(v_col)))
          is_norm_0 = tf.cast(tf.equal(v_norm2, 0), dtype=dtype)
          v_normalized.append(
              tf.expand_dims(v_col / (v_norm2 + 1 * is_norm_0), -1))
        v = tf.concat(v_normalized, axis=-1)
        # transpose, so that it is columnwise
        v = tf.transpose(v)
      else:
        v_norm2 = tf.sqrt(tf.reduce_sum(tf.square(v)))
        is_norm_0 = tf.cast(tf.equal(v_norm2, 0), dtype=dtype)
        v = v / (v_norm2 + 1 * is_norm_0)
        if 'spec_' in weight_topology_in:
          # v = MxN and U = MxN, sigma= NxN, V = NxN (sigma is vector)
          sigma, svd_u, svd_v = tf.linalg.svd(v)
          if weight_topology_in[-1] == 'U':
            v = svd_u
          elif weight_topology_in[-1] == 'S':
            v = sigma
          elif weight_topology_in[-1] == 'V':
            v = svd_v
      # Scaling
      v_scale = input_scale
      if get_mask:
        ...
        return v * v_scale  # / (v_norm2 + 1 * is_norm_0)

    v_init = ESNIn

  elif method in ['esn_res_uniform', 'esn_res_gaussian']:
    # pylint: disable=unused-argument
    def ESNRes(shape, dtype, partition_info):
      """ESN Res function."""
      # For ESN reservoir. Note scale in this case constrols the density of
      # the reservoir matrix.
      if method == 'esn_res_uniform':
        v = tf.random.uniform(
            shape, minval=-1.0, maxval=1.0, seed=wt_seed + 9, dtype=dtype)
      elif method == 'esn_res_gaussian':
        v = tf.random.normal(
            shape, mean=0.0, stddev=1.0, seed=wt_seed + 9, dtype=dtype)
      # getting the topology matrix and mask
      v, mask = GetWeightTopologyMask(v, shape, dtype, weight_topology_res,
                                      'RES', scale, wt_seed)
      v = tf.multiply(v, mask)
      # Postprocessing step
      if weight_topology_res in ['random_basis']:  # as a vector
        v_normalized = []  # vectors are stored columnwise
        for rot_num in range(shape[-1]):
          v_col = v[:, rot_num]
          # convert to matrix
          v_mat = tf.linalg.diag(v_col)
          v_mat = tf.roll(v_mat, shift=[rot_num, 0], axis=[0, 1])
          # normalize using the eigenvalues
          eigvals, _ = tf.linalg.eigh(v_mat)
          # Largest eigen val.
          v_mat /= eigvals[-1]
          # convert back to vector
          v_vec = tf.roll(v_mat, shift=[-1 * rot_num, 0], axis=[0, 1])
          # extract the diagonal
          v_vec = tf.linalg.diag_part(v_vec)
          v_normalized.append(tf.expand_dims(v_vec, -1))
        v = tf.concat(v_normalized, axis=-1)
        # transpose to make it columnwise
        v = tf.transpose(v)
      elif weight_topology_res in ['basis3D']:
        v_3d = []
        for num_b in range(shape[-1]):
          eigvals, _ = tf.linalg.eigh(v[:, :, num_b])
          # Largest eigen val.
          v_3d.append(tf.expand_dims(v[:, :, num_b] / eigvals[-1], -1))
        v = tf.concat(v_3d, axis=-1)
      else:
        eigvals, eigvecs = tf.linalg.eigh(v)
        # Largest eigen val.
        v /= eigvals[-1]
        if 'spec_' in weight_topology_res:
          # return eigvec or eigvals : X Lambda X^T
          # eigvals, eigvecs = tf.linalg.eigh(v)
          if weight_topology_res[-1] == 'X':
            v = eigvecs
          elif weight_topology_res[-1] == 'L':
            v = eigvals / eigvals[-1]
      # Scaling.
      v_scale = spectral_radius
      if get_mask:
        return mask
      else:
        return v * v_scale
  ...
  elif method in ['lstm_gaussian']:
    # pylint: disable=unused-argument
    def LSTMGaussain(shape, dtype, partition_info):
      """LSTM Gaussian function."""
      dim0 = shape[0]
      dim1 = shape[1]
      h_size = dim0 - input_nodes
      shape_input = (input_nodes, dim1)
      shape_hidden = (h_size, dim1)

      # prepare hidden matrix H*4H
      millis = int(round(time.time() * 1000))
      v = tf.random_normal(
          shape_hidden, mean=0.0, stddev=1.0, seed=millis, dtype=dtype)
      mask = tf.cast(
          tf.less_equal(
              tf.random_uniform(shape_hidden, seed=millis + 2),
              hidden_sparsity),
          dtype=dtype)
      v = tf.multiply(v, mask)
      v_norm2 = tf.sqrt(tf.reduce_sum(tf.square(v)))
      is_norm_0 = tf.cast(tf.equal(v_norm2, 0), dtype=dtype)
      v_scale = 1.0
      h_matrix = v * v_scale / (v_norm2 + 1 * is_norm_0)

      v = tf.random_normal(
          shape_input, mean=0.0, stddev=1.0, seed=millis + 4, dtype=dtype)
      mask = tf.cast(
          tf.less_equal(
              tf.random_uniform(shape_input, seed=millis + 6), input_sparsity),
          dtype=dtype)
      input_matrix = tf.multiply(v, mask)
      return tf.concat([input_matrix, h_matrix], 0)

    v_init = LSTMGaussain

  elif method in ['lstm_uniform']:
    # pylint: disable=unused-argument
    def LSTMUniform(shape, dtype, partition_info):
      """LSTM uniform function."""
      dim0 = shape[0]
      dim1 = shape[1]
      h_size = dim0 - input_nodes
      shape_input = (input_nodes, dim1)
      ...
      h_matrices = []
      for i in range(4):
        shape_hidden = (h_size, h_size)
        # prepare hidden matrix H*4H in four pieces
        millis = int(round(time.time() * 1000))
        v = tf.random_uniform(
            shape_hidden, minval=-1.0, maxval=1.0, seed=i + millis, dtype=dtype)
        mask = tf.cast(
            tf.less_equal(
                tf.random_uniform(shape_hidden, seed=1 + i + millis),
                hidden_sparsity),
            dtype=dtype)
        v = tf.multiply(v, mask)
        eigvals, _ = tf.linalg.eigh(v)
        # Largest eigen val.
        v /= eigvals[-1]
        # Scaling.
        v_scale = spectral_radius
        h_matrices.append(v * v_scale)
      h_matrix = tf.concat(h_matrices, 1)

      millis = int(round(time.time() * 1000))

      v = tf.random_uniform(
          shape_input, minval=-1.0, maxval=1.0, seed=millis + 5, dtype=dtype)
      mask = tf.cast(
          tf.less_equal(
              tf.random_uniform(shape_input, seed=millis + 7), input_sparsity),
          dtype=dtype)
      v = tf.multiply(v, mask)
      v_norm2 = tf.sqrt(tf.reduce_sum(tf.square(v)))
      is_norm_0 = tf.cast(tf.equal(v_norm2, 0), dtype=dtype)
      # Scaling
      v_scale = input_scale
      input_matrix = v * v_scale / (v_norm2 + 1 * is_norm_0)
      return tf.concat([input_matrix, h_matrix], 0)
...
def ReadFileLines(file_path):
  ...
  Args:
    file_path: path to file, either absolute or relative to the blaze workspace.
