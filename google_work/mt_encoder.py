...
class MTEncoderBiRNN(base_layer.BaseLayer):
  ...
  def __init__(self, params):
    ...
    with tf.variable_scope(p.name):
      ...
      if (p.have_projection) or (p.lstm_cell_size * 2 != p.encoder_out_dim):
        # Project the encoder output to the desired dim.
        proj_p = p.proj_tpl.Copy().Set(
            name='proj',
            batch_norm=False,
            input_dim=p.lstm_cell_size * 2,  #  2*1536
  ...
  def FProp(self, theta, input_batch):
    ...
    with tf.name_scope(p.name):
      ...
      if p.is_average_esn:
        xs = tf.math.add_n(outputs_list) / p.num_lstm_layers
