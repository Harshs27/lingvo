...
from google3.third_party.tensorflow.python.tpu import tpu_embedding as tpu_embedding_lib
...
class ProjectionLayer(quant_utils.QuantizableLayer):
  ...
  def Params(cls):
    ...
    ######## For ESN experiments #########
    p.Define('trainable', True, 'Trainable params or not.')
  ...
  def __init__(self, params):
    ...
    with tf.variable_scope(p.name):
      self.CreateVariable('w', w_pc, trainable=p.trainable)
      if p.has_bias:
        self.CreateVariable('b', b_pc, trainable=p.trainable)
      if p.weight_norm:
        self.CreateVariable('g', g_pc, trainable=p.trainable)
...
class SimpleEmbeddingLayer(quant_utils.QuantizableLayer):
  ...
  def Params(cls):
    ...
    p.Define('apply_pruning', False,
             'Whether to prune the weights while training')
    p.Define('use_basis', False,
             'Whether to use decomposed basis and coefficients matrices')
    p.Define('basis_rank', 100, 'Dimensions of basis vectors')
    p.Define('basis_init', py_utils.WeightInit.Gaussian(),
             'Dimensions of basis vectors')
    p.Define('trainable', True, 'Whether emebddings are trainable or not')
    p.Define(
        'scale_sqrt_depth', False, 'If set True, activations are scaled'
        ' with sqrt(embedding_dim) in EmbLookup.')
    return p
  ...
  def __init__(self, params):
    ...
    with tf.variable_scope(p.name):
      # Define weights
      if p.use_basis:
        basis_pc = py_utils.WeightParams([p.basis_rank] + emb_shape_suf,
                                         p.basis_init, p.dtype)
        self.CreateVariable('basis', basis_pc, theta_fn=None, trainable=False)
        pc = py_utils.WeightParams(
            shape=[p.vocab_size, p.basis_rank],
            init=p.params_init,
            dtype=p.dtype,
            collections=[self.__class__.__name__ + '_vars'])

        def ApplyBasisFn(weight):
          return py_utils.Matmul(weight, self.vars.basis)

        self.CreateVariable('wm', pc, theta_fn=ApplyBasisFn)
        ...
        if p.apply_pruning:
          ...
          self.CreateVariable('wm', pc, trainable=p.trainable)
